...
// Last update Fri Jul 12 14:08:25 2013 louis cailleux
//
...
void	Network::doProtoByName(std::string const &protocol)
{
...
void	Network::doConnection()
{
  ...
  if (connect(this->_fd, (struct sockaddr *)&this->_sIn, sizeof(this->_sIn)) == -1)
    ...
    {
      this->_talk->sendMsg(this->_fd, "IA");
      std::cout << "Welcome, your are connected on the " << this->_ip << " server." << std::endl;
      this->_talk->sendMsg(this->_fd, this->_team);
    }
}
...
void	Network::play()
{
  mapIA			*m = new mapIA(this->_fd, false);
